    ...
    <div class="header-inner g-container f-cb">
        ...
        <div class="header-logo-sd f-fr">
            <div class="user f-fr f-pr topbar-dropmap">
                <a href="<%=request.getContextPath()%>/ucenter/" target="_blank">${user.nickName}<i
    ...
    <div class="g-sd1">
        <div class="user-msg">
            ...
            <div class="user-using">
                <ul>
                    <li><a href="<%=request.getContextPath()%>/ucenter/download_record.do" id="download_record"><i
                            class="icona-download3"></i>下载记录</a></li>
                    <li><a href="<%=request.getContextPath()%>/ucenter/generate_record.do" id="generate_record"><i
                            class="icona-zjjl"></i>组卷记录</a></li>
                    <li><a href="<%=request.getContextPath()%>/ucenter/test_record.do" id="test_record"><i
                            class="icona-csjl"></i>测试记录</a></li>
                <%--    <li><a href="<%=request.getContextPath()%>/ucenter/error_record.do" id="error_record"><i
                            class="dj-error"></i>纠错记录</a></li>--%>
                    <c:if test="${loginUser.userType=='1'}">
                        <li><a href="<%=request.getContextPath()%>/ucenter/question_in_record.do"><i
                                class="icona-dtk"></i>试题录入记录</a></li>
                    </c:if>
                    <li><a href="<%=request.getContextPath()%>/ucenter/error_question.do" id="error_question"><i
                            class="icona-ctb"></i>错题本</a></li>
                    <li>
                        <div class="mt"><span href="javascript:;"><i class="icona-shoucang2"></i>我的收藏</span><b
                                class="icona-right2"></b></div>
                        <div class="mc">
                            <p><a href="<%=request.getContextPath()%>/ucenter/u_question.do" id="u_question"><i
                                    class="icona-ellipsesm"></i>试题收藏</a></p>
                            <p><a href="<%=request.getContextPath()%>/ucenter/u_paper.do" id="u_paper"><i
                                    class="icona-ellipsesm"></i>试卷收藏</a></p>
                        </div>
                    </li>
                    <li><a href="<%=request.getContextPath()%>/ucenter/index.do" id="index"><i class="icona-set"></i>个人信息</a>
                    </li>
                </ul>
            </div>
            <%--右部下--%>
        ...
        <div class="g-mn1c">
         ...
         <%--       <div class="layui-btn-group demoTable">
                ...
                <table class="layui-table"
                       lay-data="{width: 892, height:600, url:'<%=request.getContextPath()%>/paper/record.do?userId=${user.userId}', page:false, id:'idTest'}"
                       lay-filter="demo">
                    <thead>
                    <tr>
                        <%--TODO 后台删除--%>
                        <%--<th lay-data="{type:'checkbox', fixed: 'left'}"></th>--%>
                        <th lay-data="{field:'paperGradeName', width:92, sort: true, fixed: true}">适应年级</th>
                        <th lay-data="{field:'paperCourseName', width:94, sort: true, fixed: true}">适应学科</th>
                        <th lay-data="{field:'paperName', width:220, sort: true, fixed: true}">试卷名称</th>
                        <th lay-data="{field:'paperDifficultyDesc', width:75}">难度</th>
                        <th lay-data="{field:'total', width:86}">题目总数</th>
                        <th lay-data="{field:'score', width:106}">总分（/分）</th>
                        <th lay-data="{fixed: 'right', width:211, align:'center', toolbar: '#barDemo'}"></th>
                    </tr>
                    </thead>
                </table>
                <script type="text/html" id="barDemo" style="width: 247px">
                    <a class="layui-btn layui-btn-primary layui-btn-xs" lay-event="detail">查看</a>
                    <a class="layui-btn layui-btn-normal layui-btn-xs" lay-event="testpaper">在线测试</a>
                    <a class="layui-btn layui-btn-xs" lay-event="downloadpaper">下载试卷</a>
...
<script type="text/javascript">
    ...
    layui.use('table', function () {
        ...
        table.on('tool(demo)', function (obj) {
            ...
            if (obj.event === 'detail') {
                //查看===========================================================
                window.location.href = server_context + "/paper/view.do?paperId=" + data.paperId;
            }
            else if(obj.event ==='testpaper'){
                window.location.href = server_context + "/paper/test.do?paperId=" + data.paperId;
            }else if(obj.event ==='downloadpaper'){
                window.location.href = server_context + "/paper/download_paper_doc.do?paperId=" + data.paperId;
            }
                ...
                layer.confirm('真的删除行么', function (index) {
                    ...
                    //删除记录========================================================
                    $.ajax({
                        url: server_context + '/paper/delete.do',
                        type: 'POST', //GET
            ...
            getCheckData: function () { //获取选中数据
                ...
                layer.confirm('真的删除这' + data.length + '行么', function (index) {
                    ...
                    $.ajax({
                        ...
                        async: true,    //或false,是否异步
                        data: {
                            paperId: 0, paperIds: ids.toString()
                        },
                        timeout: 5000,    //超时时间
                        dataType: 'text',    //返回的数据格式：json/xml/html/script/jsonp/text
                        success: function (data, textStatus, jqXHR) {

            ...
            <div class="user f-fr f-pr topbar-dropmap" style="float: right">
                <c:choose>
                    <c:when test="${user!=null}">
                        <a href="<%=request.getContextPath()%>/ucenter/" target="_blank"
                           style="float: right">${user.nickName}<i
                        ...
                        <div class="drop-bd" style="display: none;">
                            <ul>
                                ...
                                <li><a href="<%=request.getContextPath()%>/ucenter/test_record.do"
                                       target="_blank">测试记录</a></li>
                                <%--<li><a href="<%=request.getContextPath()%>/ucenter/error_record.do"
                                       target="_blank">纠错记录</a>
                                </li>--%>
                                <c:if test="${loginUser.userType=='1'}">
                                    <li><a href="<%=request.getContextPath()%>/ucenter/question_in_record.do"
                                           target="_blank">试题录入记录</a>
                                    </li>
                                </c:if>
                                <li><a href="<%=request.getContextPath()%>/ucenter/error_question.do" target="_blank">错题本</a>
                                </li>
                                <li><a href="<%=request.getContextPath()%>/ucenter/myfavorite.do"
                                       target="_blank">我的收藏</a></li>
                                <li><a href="<%=request.getContextPath()%>/ucenter/index.do" target="_blank">个人信息</a>
                                </li>
                                <li class="split-line"></li>
                                <li><a data-method="post" href="<%=request.getContextPath()%>/user/logout.do">退出</a>
                                </li>
                            </ul>
                    ...
                    <c:otherwise>
                        <div class="loginbox">
                            <a href="<%=request.getContextPath()%>/user/login.do" class="login"><span
                                    style="color:#0C0C0C ">登录</span></a>
                            <a href="<%=request.getContextPath()%>/user/register.do" class="register"><span
                                    style="color:#0C0C0C ">注册</span></a>
                        </div>
                    </c:otherwise>
                </c:choose>
...
<div class="preview-con g-container">
    <div class=" f-fl f-w">
        ...
        <div class="dj-preview">
            <div class="preview-title">
                <h1>${paperTestPojo.paperName}</h1>
                <p>
                    <span><i class="icona-time2"></i>测试开始时间：<fmt:formatDate value="${paperTestPojo.startTime}"
                                                                            type="date" dateStyle="full"/></span>
                    <span><i class="icona-leixing"></i>试卷总分：${paperTestPojo.totalScore}分</span>
                    <span><i class="icona-leixing"></i>考试总时间：${paperTestPojo.costTime}分钟</span>
            ...
            <div class="preview-body" id="J_PaperBody">
                ...
                <div class="layui-progress layui-progress-big" lay-showPercent="true"
                     style="width: 70%;margin-bottom:-5px;display: inline-block">
                    <div class="layui-progress-bar" lay-percent="${timePer}%"></div>
                </div></span><br><br>
                    <span>分数占比：
                <div class="layui-progress layui-progress-big" lay-showPercent="true"
                     style="width: 70%;margin-bottom:-5px;display: inline-block">
                    <div class="layui-progress-bar" lay-percent="${scorePer}%"></div>
                </div></span><br><br>

                    <c:forEach items="${paperTestPojo.errorQuestionPojos}" var="question" varStatus="vs">
                        <c:if test="${vs.index==10}"><br><br></c:if>
                        <c:if test="${vs.index==20}"><br><br></c:if>
                        <c:if test="${vs.index==30}"><br><br></c:if>
                        <c:choose>
                            <c:when test="${question.right}">
                                <a href="<%=request.getContextPath()%>/question/detail.do?questionId=${question.questionId}" class="layui-btn">${vs.index+1}</a>
                            </c:when>
                            <c:otherwise>
                                <a href="<%=request.getContextPath()%>/question/detail.do?questionId=${question.questionId}" class="layui-btn layui-btn-danger">${vs.index+1}</a>
                            </c:otherwise>
                        </c:choose>
                    </c:forEach>
    ...
    <div class="dj-about" style="position: fixed; margin-left: 950px; z-index: 9; top: 110px;">
        ...
        <blockquote class="layui-elem-quote" style="background-color:snow;font-size: 24px;color:#09A74D ">
            测试报告：
            <div id="" style="margin-top: 15px">分数：${paperTestPojo.testScore}分</div>
            <div id="" style="margin-top: 15px">耗时：${paperTestPojo.costTime}分钟</div>
        </blockquote>
        <div class="view-toolbars f-cb">
            <a href="<%=request.getContextPath()%>/index.do" class="download-btn"><i
                    class="layui-icon">&#xe68e;
            </i>返回首页</a>
        </div>
        <script>
